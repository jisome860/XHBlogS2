...
def update_site_config(payload: Dict[str, Any] = Body(...)):
    try:
        ...
        print("\n" + "=" * 50)
        print(f"[CONFIG] 启动物理引擎，目标文件: {config_path}")
        print(f"[CONFIG] 收到更新字段: {list(updates.keys())}")
        updated_count = 0
            ...
            if key == "gitalkConfig":
                try:
                    if not isinstance(value, dict):
                        print(f"  [CONFIG] gitalkConfig 不是 dict，跳过")
                        continue
                    ...
                    if not isinstance(admin_list, list):
                        admin_list = []
                    admin_str = '["' + '", "'.join(str(a) for a in admin_list) + '"]'
    ...
    admin: {admin_str},
                    ...
                    if re.search(pattern, content):
                        ...
                        print(f"  [CONFIG] 成功修改 -> [{key}]")
                        updated_count += 1
                except Exception as e:
                    print(f"  [CONFIG] gitalkConfig 处理异常: {e}")
                continue
            ...
            try:
                ...
                if re.search(pattern, content):
                    ...
                    print(f"  [CONFIG] 未在文件中找到 [{key}]，跳过")
            except Exception as e:
                print(f"  [CONFIG] 处理 [{key}] 时异常: {e}")
                continue
        ...
        print(f"[CONFIG] 任务完成，共刷新 {updated_count} 个字段")
        print("=" * 50 + "\n")

        return {"success": True, "message": f"本地 siteConfig.ts 修改成功！已更新 {updated_count} 个字段"}
